    ...
    <div className="min-h-screen bg-cream3 flex items-center justify-center px-4">
      <div className="w-full max-w-sm animate-fade-up">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-ink3 mt-1">Sign in to DevConnect</p>
        </div>
        <div className="card p-7">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full py-3 text-sm disabled:opacity-60 mt-2">{loading ? 'Signing in...' : 'Sign in'}</button>
          </form>
        </div>
        <p className="text-center text-sm text-ink3 mt-5">No account? <Link href="/auth/register" className="text-grove font-medium no-underline">Join free →</Link></p>
      </div>
    </div>
  );
}
